...
def charmapreplace(s):
    ...
    while True:
        ...
        low = s[p:p+2] == u'#c'
        arg = s[p+2]
        assert arg in argname, arg
        s = s.replace(s[p:p+3], u"{%s %s}" % (u"color" if low else u"COLOR", argname[arg]))
    while True:
        p = s.find(u"$d")
        if p < 0: p = s.find(u"$m")
        if p < 0: p = s.find(u"$n")
        if p < 0: p = s.find(u"$i")
        if p < 0: break
        base = {u"d": u"VALUE", u"m": u"POKEMON", u"n": u"NAME", u"i": u"MOVE_ITEM"}[s[p+1]]
        arg = int(s[p+2])
        s = s.replace(s[p:p+3], u"{%s_%d}" % (base, arg))
    s = s.replace(u"$t", u"{TEAM_NAME}")
    s = s.replace(u"$h", u"{FRIEND_AREA}")
    return s
...
def main():
    ...
    cstr = lambda s: u'_("%s")' % charmapreplace(s).replace(u'"', u"\\\"").replace(u"\n",u"\\n").replace(u"~27",u"'").replace(u"~2c", u",").replace(u"~93",u"“").replace(u"~94",u"”").replace(u"~22", u"\\\"").replace(u"#+", u"{CENTER_ALIGN}").replace(u"#P", u"{EXTRA_MSG}").replace(u"#W", u"{WAIT_PRESS}").replace(u"#n", u"{NEW_LINE}").replace(u"#R", u"{RESET}").replace(u"#r", u"{reset}")
...
""" % (v[1], basename))
            ...
            for i in range(v[2]):
                ...
                if False: pass
                    ...
                    assert a2 == ap == 0
                    outfile.write(u'    EXECUTE_STATION(%d, %d, %d),\n' % (a1, h, b))
                elif op == 0x1e:
                    assert a2 == ap == 0
                    outfile.write(u'    EXECUTE_SUBSTATION(%d, %d, %d),\n' % (a1, h, b))
                elif op == 0x1f:
                    assert b == h == a1 == a2 == ap == 0
